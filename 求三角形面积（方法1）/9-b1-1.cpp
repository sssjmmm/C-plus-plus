...
public:
	...
	double area()
	{
		double a, b, c, p;
		double areas;
		a = sqrt((p2.getx() - p1.getx()) * (p2.getx() - p1.getx()) + (p2.gety() - p1.gety()) * (p2.gety() - p1.gety()));
		b = sqrt((p3.getx() - p1.getx()) * (p3.getx() - p1.getx()) + (p3.gety() - p1.gety()) * (p3.gety() - p1.gety()));
		c = sqrt((p3.getx() - p2.getx()) * (p3.getx() - p2.getx()) + (p3.gety() - p2.gety()) * (p3.gety() - p2.gety()));
		p = (a + b + c) / 2;
		areas = sqrt(p * (p - a) * (p - b) * (p - c));
		if (fabs(a + b - c) < 1e-6 || fabs(a + c - b) < 1e-6 || fabs(c + b - a) < 1e-6)
			return -1;
		else
		return areas;
